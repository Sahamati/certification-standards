Feature: On calling POST Consent Notification API, verify that on sending valid consent Notification
  with 'PAUSED' status, user is not able to do FI Request.

  Scenario: 1016_1 Set the Consent post request response, and Consent Handle request response with
  ...
  Finally Send the Consent Notification with  consent Status as 'PAUSED' and  ask the user if he is able
